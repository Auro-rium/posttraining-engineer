...
class ObjectiveService:
    ...
    def __init__(self, engine: ServiceRecoveryEngine, auth_token: str) -> None:
        if not auth_token:
    ...
    def benchmark(self, request: BenchmarkRequest) -> BenchmarkResponse:
        ...
        for task_id in request.task_ids:
            trajectory = self.engine.run_episode(task_id, [], split=request.split)
        ...
        benchmark_id = (
            "benchmark-"
            + hashlib.sha256(
                f"{request.run_id}:{request.split.value}:{','.join(request.task_ids)}".encode()
            ).hexdigest()[:24]
        )
...
def create_objective_app(
    ...
    *,
    ...
    service = ObjectiveService(engine or ServiceRecoveryEngine(), auth_token)
