...
coordinator/tool-provided `evidence_class`, `champion_metrics`, `candidate_metrics`,
`regression_metrics`, `provider_job_ids`, `provenance`, and `errors`. Copy numeric measurements and
provenance only from the evaluation worker response; missing values stay missing or block the result.
Evaluation does not decide promotion.
